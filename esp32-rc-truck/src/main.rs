...
fn main() -> anyhow::Result<()> {
    ...
    let throttle_and_steering_and_tray_timer_driver = std::sync::Arc::new(LedcTimerDriver::new(peripherals.ledc.timer0, &throttle_and_steering_timer_config)?);

    let throttle_driver = LedcDriver::new(peripherals.ledc.channel0, std::sync::Arc::clone(&throttle_and_steering_and_tray_timer_driver), peripherals.pins.gpio4)?;
    let steering_driver = LedcDriver::new(peripherals.ledc.channel1, std::sync::Arc::clone(&throttle_and_steering_and_tray_timer_driver), peripherals.pins.gpio5)?;
    let tray_driver = LedcDriver::new(peripherals.ledc.channel2, std::sync::Arc::clone(&throttle_and_steering_and_tray_timer_driver), peripherals.pins.gpio14)?;
