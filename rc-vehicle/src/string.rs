...
impl crate::vehicle::InputMessageHandler for StringFormatTank {
    fn handle_input_message(&mut self, input_message: rc_messaging::serialization::InputMessage) -> anyhow::Result<()> {
        ...
        println!("throttle_left={:?}, throttle_right={:?} from input_message={:?}", throttle_left, throttle_right, input_message);

        self.string_format_handler.set_throttles(throttle_left, throttle_right)?;
...
mod tests {
    ...
    fn get_test_resources() -> (
        ...
        let string_format_tank = StringFormatTank::new(
            false,
            Box::new(test_string_car),
        );

        return (drain_input_states, string_format_tank);
    }
